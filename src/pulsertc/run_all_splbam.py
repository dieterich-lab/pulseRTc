...
def main():
    ...
    vcf_str = ""
    if "vcf" in config.keys() and config["vcf"]:
        vcf_str = "--vcf"
    if "vcf" in config.keys() and "snpdata" not in config.keys():
    ...
    if args.library_type == "infer":
        ...
        exist = utils.check_files_exist(
            [bamf, args.gtf], raise_on_error=True, logger=logger
        )

        msg = (
            f"Library type inferred using {bamf}, with first {args.sample_size} gene records "
